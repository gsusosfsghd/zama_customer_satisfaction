...
function useMetaMaskInternal(): UseMetaMaskState {
  ...
  const persistWalletState = useCallback(
    async (
      ...
      if (accounts && accounts.length > 0) {
        await storage.setItem("wallet.lastAccounts", JSON.stringify(accounts));
      }
      if (chainId) {
        await storage.setItem("wallet.lastChainId", chainId.toString());
      }
      await storage.setItem("wallet.connected", connected.toString());
    },
    [storage]
  );

  // Restore wallet state on mount (silent reconnect using eth_accounts)
  useEffect(() => {
    const restoreWallet = async () => {
      try {
        ...
        if (
          ...
          if (providerDetail) {
            try {
              // Silent reconnect using eth_accounts (no user prompt)
              const accountsArray = await providerDetail.provider.request({
                method: "eth_accounts",
              });
              ...
              if (accountsArray && accountsArray.length > 0) {
                ...
                await persistWalletState(
                  lastConnectorId,
                  accountsArray as string[],
                  Number.parseInt(chainIdHex as string, 16),
                  true
                );
              }
            } catch (e) {
              console.log("[useMetaMask] Silent reconnect failed:", e);
            }
          }
        }
      } catch (e) {
        console.log("[useMetaMask] Restore wallet failed:", e);
      }
    };
  ...
  }, [providers, storage, persistWalletState]);
  ...
  useEffect(() => {
    ...
    if (next) {
      // Connect
      nextConnectListener = async (connectInfo: ProviderConnectInfo) => {
      ...
      // Disconnect
      nextDisconnectListener = async (error: ProviderRpcError) => {
        if (next !== metaMaskProviderRef.current) {
          return;
        }
        ...
        _setChainId(undefined);
        _setAccounts(undefined);
        await persistWalletState(undefined, undefined, undefined, false);
      };
      disconnectListenerRef.current = nextDisconnectListener;

      // ChainChanged
      nextChainChangedListener = async (chainId: string) => {
        if (next !== metaMaskProviderRef.current) {
          return;
        }
        ...
        _setCurrentProvider(next);
        _setChainId(chainIdNum);
        await persistWalletState(undefined, accounts, chainIdNum, true);
      };
      chainChangedListenerRef.current = nextChainChangedListener;

      // AccountsChanged
      nextAccountsChangedListener = async (accounts: string[]) => {
        if (next !== metaMaskProviderRef.current) {
          return;
        }
        console.log(
  ...
  }, [providers, accounts, chainId, persistWalletState]);
